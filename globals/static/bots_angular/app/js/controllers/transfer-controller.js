angular.module('InventoryApp.TransferCtrl', []).
controller('TransferCtrl', ['$scope', '$http', "$location", "$filter", "ServerDataService", "UtilsService", "UserPrefsService", function($scope, $http,$location,$filter,ServerDataService, UtilsService,UserPrefsService) {
    $(".input_date").focus();
    $scope.total_price = 0;
    $scope.page_price = 0;
    $scope.limit = 100;
    ...
    $scope.load_patient = function(location_id){
        $scope.patient = undefined;
        var url = "/api/v1/patient/?location=" + location_id +"&format=json";
        $http.get(url).success(function(data){
            if (data.meta.total_count === 1){
                $scope.patient = data["objects"][0];
            }
        });
    };
    var initialize_transfer = function(){
        ...
        if ("shipment" in $location.search()){
            ...
            ServerDataService.get("shipment",$location.search()['shipment']).then(function(data){
                ...
                if ($scope.shipment.to_location.location_type==="P"){
                    $scope.dispense = true;
                    $scope.load_patient($scope.shipment.to_location.id);
                }
                ...
                if ("quicktransfer" in $location.search()){
                    var url = "/api/v1/item/" + Number($location.search()["item"]) +"/?format=json";
                    $http.get(url).success(function(data){
                        $scope.edit_item(data);
                    });
                }
        ...
        else if ("quicktransfer" in $location.search()){
            $scope.quick_transfer = true;
            $scope.edit_item_id = Number($location.search()["item"]);
            $scope.shipment = {};
            $scope.shipment.date = "t";
            $scope.submit_from_location({"id" : Number($location.search()["from_location"])});
            $scope.submit_to_location({"id" : Number($location.search()["to_location"])});
            $scope.show_results = true;
        }
    ...
    $scope.edit_patient_details = function(){
    ...
    $scope.submit_to_location = function(location){
        ...
        // get patient data if dispensing
        if($scope.dispense){
            $scope.load_patient(location.id);
        }
        $scope.editing_to_location = false;
        $scope.show_to_location_query = false;
    };
    $scope.change_district = function(){
        $scope.patient.district = undefined;
    };
    $scope.submit_district = function(district){
        $scope.patient.district = district;
    };
    $scope.add_to_location_toggle = function(to_location){
        $scope.new_to_location_name = to_location;
        $("#add_to_location_modal").modal();
    };
    $scope.add_district = function(district){
        if (district){
            var district = {"name" : district};
            ServerDataService.save('district',district).then(function(data){
                $scope.patient.district = data;
            });
        }
    };
    $scope.add_patient_toggle = function(identifier){
        $scope.patient = {};
        $scope.patient.editing_identifier = identifier;
        $("#add_patient_modal").modal();
    ...
    $scope.patient_search_config = {
        ...
        url : "/api/v1/location/?format=json&order_by=name&search_patients=true",
        result_name : "patient",
        input_class : "patient_input",
        select_result: $scope.submit_to_location,
        add_results : true,
        add_result : $scope.add_patient_toggle
    };
    ...
    $scope.check_name = function(name){
        $scope.patient.warn_name = false;
        var url = "/api/v1/patient/?name=" + name +"&format=json";
        $http.get(url).success(function(data){
            if (data.meta.total_count > 0){
                $scope.patient.warn_name = true;
            }
            else{
                $scope.patient.warn_name = false;
            }
            $scope.patient.name = $scope.patient.editing_name;
        });
    };
    ...
    $scope.create_patient = function(patient){
        if (!patient.identifier && patient.editing_identifier){
            patient.identifier = patient.editing_identifier;
        }
        if (patient.identifier){
            ...
            if ($scope.patient.district){
                p['district'] = "/api/v1/district/" + $scope.patient.district.id + "/";
            }
            if ($scope.patient.location){
                p['location'] = "/api/v1/location/" + $scope.patient.location.id + "/";
            }
            ServerDataService.save('patient',p).then(function(data){
                $scope.shipment.to_location = data.location;
                $scope.patient = data;
    ...
    $scope.edit_item = function(item){
        ...
        $("#qty").focus();
        var soh_date = serialize_date(new Date($scope.shipment.date));
