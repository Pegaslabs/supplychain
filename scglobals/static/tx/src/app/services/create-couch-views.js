  ...
  constructor(db){
    ...
    this.couch_views = [
      ...
      // for inventory
      {
        ...
        mapFun: function(doc){
            if (doc.doc_type && doc.doc_type === "shipment"){
              // see all-transactions comment
              doc.transactions.forEach(function(transaction,i){
                emit([doc.from_location_name,transaction.item_name,transaction.item_category_name,transaction.item_lot_expiration,transaction.item_lot_num], -1*transaction.qty);
                emit([doc.to_location_name,transaction.item_name,transaction.item_category_name,transaction.item_lot_expiration,transaction.item_lot_num], transaction.qty);
              });
            }
        }
      },
